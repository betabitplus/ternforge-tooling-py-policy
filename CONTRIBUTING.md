...
Use [docs/py_lib_policy/README.md](docs/py_lib_policy/README.md) for package
docs, [tests/README.md](tests/README.md) for test-tree layout, and
[docs/py_lib_policy/verification/README.md](docs/py_lib_policy/verification/README.md)
...
```bash
uv run pytest tests/py_lib_policy -m hermetic
```
...
```bash
direnv exec . uv run python examples/py_lib_policy/<module>.py
```

Keep examples focused on imports from `py_lib_policy`. If an example
...
```bash
direnv exec . uv run python -m workbench.py_lib_policy.<module>
```
...
direnv exec . uv run python scripts/reproduce_running_loop.py \
    workbench.py_lib_policy.<module>
```

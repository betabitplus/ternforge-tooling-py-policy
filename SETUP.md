...
```bash
direnv exec . uv run pytest tests/py_lib_policy -m hermetic -q
direnv exec . uv run pytest tests/py_lib_policy/integration/test_config_lifecycle.py -q --no-cov
```
...
and reselect `py-lib-policy (.venv)`.
